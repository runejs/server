...
export function openShop(player: Player, identification: string, closeOnWalk: boolean = true): void {
    try {
        ...
        player.packetSender.updateInterfaceString(3901, openedShop.name);
        for(let i = 0; i < 30; i++) {
            if(openedShop.items.length <= i) {
                player.packetSender.sendUpdateSingleInterfaceItem(3900, i, null);
            } else {
                player.packetSender.sendUpdateSingleInterfaceItem(3900, i, {
